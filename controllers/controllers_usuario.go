...
var InsertUsuario = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request){
	...
	defer file.Close()
	f, err := os.OpenFile("/home/zaptec/img/"+hadler.Filename, os.O_WRONLY|os.O_CREATE, 0666) 
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	*/	
	...
	sqlQuery := "INSERT INTO public.usuario(nome,email,senha,cpf,celular,sexo,perfil_id) VALUES($1,$2,$3,$4,$5,$6,$7,$8)"
	row, err := connectingDB.Exec(sqlQuery,usuario.Nome,usuario.Email,
									usuario.Senha,usuario.Cpf,usuario.Celular,usuario.Sexo,usuario.Perfil)
	if err != nil {
